...
async def on_message(message):
    ...
    async def play_next():
        ...
        if not playing:
            try:
                ...
                voice_clients[message.guild.id].play(player, after=lambda e: asyncio.run(play_next()))

                await message.channel.send(f'Now playing: **{current_song["title"]}**\n{current_song["webpage_url"]}')         
    ...
    if message.content.startswith('?search'):
        ...
        try:
            info = YT_DL.extract_info("ytsearch:%s" % message.content.split('?search ')[1:], download=False)["entries"][0]
        ...
        try:
            if len(queue) > 0:
                ...
                try:
                    ...
                    playing = False
                    await message.channel.send('There is no song left in the queue')
    ...
    elif 'artur' in message.content.lower():
        await message.channel.send('Ser de azul e do Porto')
